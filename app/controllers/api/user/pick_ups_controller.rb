class Api::User::PickUpsController < Api::UserController
  ...
  def upcoming
    @pick_ups = current_user.pick_ups.where(status: [PickUp::STATUSES[:accepted], PickUp::STATUSES[:pending]]).where("start_time > ?", Time.now).order(start_time: :asc)
  ...
  def history
    @pick_ups = current_user.pick_ups.where(status: [PickUp::STATUSES[:done], PickUp::STATUSES[:canceled]]).order(start_time: :asc)
    @pick_ups = @pick_ups.page(params[:page]).per(params[:per_page] || 10)
    render  json: @pick_ups,
              each_serializer: User::PickupSerializer,
              meta: {
